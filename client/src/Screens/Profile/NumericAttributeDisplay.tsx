...
type NumericAttributeDisplayProps = {
  label: string;
  value?: number;
  updateFunc: (newValue: number) => Promise<ModifyUserProfileResp>;
  setUserProfile: React.Dispatch<React.SetStateAction<UserProfile | undefined>>;
};

function NumericAttributeDisplay({
  label,
  value,
  updateFunc,
  setUserProfile
}: NumericAttributeDisplayProps) {
  const [editing, setEditing] = useState(false);
  const [inputValue, setInputValue] = useState<string>(value?.toString() ?? "");

  const handleSave = async () => {
    try {
      const resp = await updateFunc(Number(inputValue));
...
export default NumericAttributeDisplay;
